...
The R0 compact architecture and invariant index (README §12). Everything here is stated once and links to the governing text; rationale lives in the numbered documents. Scope: what an implementer of **chapter one** (`00a-chapter-one-content.md`) must know. Limits: under 10k tokens, 39 named concepts (§3 is the complete list; capability ids such as `tide@1` and scenario ids are not concepts).

## 1. Provenance

| Item | Value |
|---|---|
| Packet state | Draft 0.5, R0 acceptance pending; built from `main` after commit `e6a8348` |
...
| 22 | **Fact** | typed, scoped, defaulted narrative value; the coordination channel between quests and world | 03 §7; 06 §11 |
| 23 | **Quest** | pure reducer over committed DomainEvents with objective operators and typed outcome consequences | 06 §1–5, §10 |
...
| 31 | **ReactionRule** | typed rule: on committed event pattern, under Policy, emit registered consequences; chains bounded | 06 §14; 21 §11 |
...
| 35 | **Connection / Barrier** | reciprocal exit in one of six directions; a Barrier is one shared state seen from both faces | 21 §5; 05 §17 |
...
| A3 | UI/text/bots emit ActionInvocation; the authority re-resolves and revalidates against committed state; hidden or stale actions reject | 04 §1–2 | 009 | ACT-09, ACT-10, MODE-04 |
...
| A5 | Decision output is a proposal until commit; on rejection or failed commit nothing escapes and nothing advances | 04 §5.1; 07 §6 | 059 | ARCH-09, DET-09, DET-10 |
...
| D5 | A kernel fault is contained at the host boundary with a complete repro record; it never commits partial state | 11 §7; 09 §2 | 033 | DET-11, OFF-03, OFF-07 |
...
| P8 | A new app or kernel opens every previously valid installed save, offline, with rollback-safe migration | 10 §22, §28 | 022 | COMPAT-01, COMPAT-02, COMPAT-03, COMPAT-07, MODE-09, OFF-11 |
...
| C5 | Content gains expressivity only by composition; no hidden persistence, host callbacks, second mutation authority, `Code.eval_string`, or downloaded executable code | 01 P2a; 21 §1; 10 §14 | 050, 018 | ARCH-04, ARCH-08, ACTIONRECIPE-01 |
...
| # | Invariant | Doc § | ADR | Scenarios |
|---|---|---|---|---|
| N1 | A Quest reduces committed DomainEvents only; pre-activation events do not credit; duplicate events are idempotent | 06 §1, §5 | 017 | QST-05, QST-06, QST-30 |
...
| N9 | A Fact is typed, scoped, defaulted, and the only coordination channel between quests and world systems | 03 §7; 06 §11 | 041 | QST-21, QST-22 |

### Living world

| # | Invariant | Doc § | ADR | Scenarios |
|---|---|---|---|---|
| W1 | Exits are reciprocal and use only north, south, east, west, up, down; a Barrier has one state shared by both faces | 21 §5; 05 §17; 00 §4.1 | 051 | BARRIER-01, WORLD-02 |
| W2 | Time-derived state (tide, light, shop hours, cooldown, regen) is derived from the logical clock, never ticked or stored; side-effecting deadlines are durable jobs; ephemeral timers are recreated | 02 §11; 21 §28 | 031 | WORLD-06, WORLD-07, QST-09 |
...
| M2 | Save on every action; a kill at any point resumes from the last commit with no duplication or loss | 00 §1; 07 §9 | 003 | OFF-03, OFF-04, OFF-05, OFF-06, OFF-07, INV-02 |
...
Ladder rules: a mechanic is built in the chapter that first needs it; a chapter-one save opens in the chapter-one release forever; three artifacts, three hashes; R10 means chapter one, R16 means chapters two and three plus R9C.

## 6. Phase gates

Source: 14. Sizing is for one developer with agent assistance, revised after R1 and R6.
...
| R7 Quest, dialogue, scenes | Lokacore quest-bug regression cannot reproduce; a quest drives a durable scene with choice, crash checkpoint, and exactly-once consequences; a dream proves overlay + InstancePlan; no LokaScript | 4 to 6 weeks |
...
| **Chapter one in the store** | R10 + R12 + R13 | **about 9 to 14 months from R1** |
| R11 Builder v1 | an agent recreates R10 content through Builder tools only | 4 to 8 weeks |
| R14–R22 Realm | online authority, deployments, factory, party, social shell, instanced regions, shards, promotion, MMO | not sized; after chapter one |
